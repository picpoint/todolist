<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="stylesheet" href="public/css/style.css">
    <title>Todo List</title>
</head>
<body>

    <div class="indx__wrapper">
        <div class="indx__header">
            <div class="indx__headermenu">
                <div class="indx__logoblock">
                    <a href="index.php"><img src="../public/pict/logo.png" alt="logo"></a>
                </div>
                <div class="indx__personal">
                    <a href="../registration.php">Регистрация</a>
                    <a href="#">Авторизация</a>
                </div>
            </div>
        </div>
        <div class="indx__body">
            <img src="../public/pict/todo.png" alt="todo">
        </div>
        <div class="indx__footer">
            <span>Все права не защищены. Всех прав не защитить.</span>
        </div>
    </div>

</body>
</html>